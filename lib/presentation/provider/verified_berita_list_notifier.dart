...
class VerifiedBeritaListNotifier extends ChangeNotifier {
  List<BeritaEntities> _listberita = [];
  List<BeritaEntities> get listberita => _listberita;
  String _message = "";
  String get message => _message;

  VerifiedBeritaListNotifier({required this.getallberita});

  final GetAllBerita getallberita;

  Future fetchdatalistberita() async {
    try {
      Logger().d("fetch data verified diajalankan");
      final data = await getallberita.execute(type: BeritaType.verified);
      Logger().d("data verified nya adalah ${data.length}");
      if (data.isNotEmpty) {
        _listberita = data;
